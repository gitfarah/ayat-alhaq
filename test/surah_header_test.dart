...
void main() {
  ...
  group('Surah header bands', () {
    setUpAll(() async => SurahHeaderService.load());

    /// Every band in the shipped asset, flattened.
    List<SurahHeaderBand> allBands() {
      final out = <SurahHeaderBand>[];
      for (var p = 1; p <= 604; p++) {
        out.addAll(SurahHeaderService.forPage(p));
      }
      return out;
    }
    ...
    test('page 604 carries the three closing surahs', () {
      final bands = SurahHeaderService.forPage(604);
    ...
    test('a page with no surah start has no bands', () {
      ...
      expect(SurahHeaderService.forPage(10), isEmpty);
    });
  });
